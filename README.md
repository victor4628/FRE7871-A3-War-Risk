...
The final application covers **178 sessions, 1,029 eligible headlines, 515 war-relevant headlines, and 18 matched event/control pairs**. The Treasury anchor is weak (IV1 robust first-stage F **0.886**; its second-moment shift has a block-bootstrap interval spanning zero). No signed-news association survives false-discovery correction. The conditional replication estimates are **exploratory**, not reliable causal Iran war-risk sensitivities. This analysis was not preregistered.

## Deliverables

- `output/pdf/iran_war_risk_2026.pdf`: methodology, findings, replication tables, diagnostics, and event/control appendix.
- `output/analysis/table2_sensitivities.csv`: the two covariance-ratio estimates and combined-instrument estimate, normalized to a 25 bp fall in the US two-year yield; confidence and identification diagnostics.
...
- FRED series pages: `https://fred.stlouisfed.org/series/<SERIES_ID>`; providers include the Federal Reserve, EIA, S&P Dow Jones, ICE BofA, CBOE, and Nikkei.
- Public ETF histories: Yahoo Finance daily chart responses for GLD, ACWI, FEZ, and EEM. Adjusted closes are used. GLD is an ETF return proxy for gold, not a dollar bullion-price response; ACWI/FEZ/EEM are US-listed USD instruments with fees, distributions, and market timing differences.
...
Treasury constant-maturity yields replace estimated off-the-run par yields; ICE option-adjusted spreads replace the original credit spreads; spot Brent/WTI replace year-ahead oil futures. No comparable public 2026 on-the-run/off-the-run liquidity-premium series was obtained, so that original outcome is explicitly omitted. The broad dollar definition and gold measurement also differ. Nikkei closes before most US trading and are shown as a timing-limited extension. The 2026 sample includes both anticipation and realized war, whereas the original main sample focused on the buildup to the Iraq war.
