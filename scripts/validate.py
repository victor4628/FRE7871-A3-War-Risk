...
for loading in [2.,-3.]:
    ...
    d,e1,e2=cov_stats(a,b);e3,se=iv(a,b,'both');ez,_=iv(a,b,'z1');ez2,_=iv(a,b,'z2')
    ...
    assert abs(ez2-e2)<1e-10,'Instrument two must equal covariance ratio'
    assert se>0
...
assert set(changes.columns)=={'DGS2','DGS10','T10YIE','SP500','BAMLC0A4CBBB','BAMLH0A0HYM2','DCOILBRENTEU','GLD','DTWEXBGS'}
assert tables[['e1_robust_se_effect','e2_robust_se_effect','e3_robust_se_effect']].notna().all().all()
assert (tables[['e1_robust_se_effect','e2_robust_se_effect','e3_robust_se_effect']]>=0).all().all()
assert s['archive_failures']==0,'Complete the news archive before delivery'
assert s['US_sessions']==len(changes)
result={'status':'passed','checks':['Known positive and negative factor loadings recovered in simulation','IV1 and IV2 equal their difference-of-second-moment estimators','Robust standard errors produced for IV1, IV2, and both instruments','Main financial-variable set includes eight comparable outcomes and the Treasury anchor','Disjoint unique event and control days','Macro dates excluded and prewar/wartime matching enforced','Yield units and shock normalization checked','News archive coverage complete','Sample end obeyed'],'matched_pairs':len(p),'financial_variables':len(changes.columns)}
